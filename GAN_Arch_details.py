...
class CycleGAN_Discriminator(nn.Module):
    def __init__(self, filter=16):
        ...
        self.block = nn.Sequential(
            ...
            Downsample(filter * 4, filter * 8, kernel_size=4, stride=2),
            Downsample(filter * 8, filter * 16, kernel_size=4, stride=1),
        )

        self.last = SelfONN1dlayer(filter * 16, 1, kernel_size=4, stride=2, padding=1,q=5)
