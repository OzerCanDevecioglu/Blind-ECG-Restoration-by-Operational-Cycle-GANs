...
class ECGDataset(Dataset):
    def __init__(self, base_img_paths, style_img_paths, phase='train'):
        ...
        self.phase = phase
        path_data_ch1 = loadmat('mats/clean_sig.mat')
        label = loadmat('mats/noisy_sig.mat')
        self.data_ch1=path_data_ch1["clean_sig"]

        
        self.label=label["noisy_sig"]


        self.data=np.zeros((1,int(len(self.data_ch1)),4000))
        self.data2=np.zeros((1,int(len(self.label)),4000))
...
class TECGDataset(Dataset):
    def __init__(self, base_img_paths, style_img_paths, phase='train'):
        ...
        self.data_ch1=path_data_ch1["noisy_sig"]
...
class TECGDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self):
        self.base_img_paths = glob.glob(os.path.join(self.data_dir, 'noisy_sig.mat'))
        self.style_img_paths = glob.glob(os.path.join(self.data_dir, 'noisy_sig.mat'))
